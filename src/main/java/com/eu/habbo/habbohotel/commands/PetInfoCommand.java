...
public class PetInfoCommand extends Command implements TIntObjectProcedure<AbstractPet>
{
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception
    {
        if(params.length > 1)
        {
            ...
            gameClient.getHabbo().getHabboInfo().getCurrentRoom().getCurrentPets().forEachEntry(this);
        }
        else
        {
            gameClient.sendResponse(new RoomUserWhisperComposer(new RoomChatMessage("Please provide a pet username!", gameClient.getHabbo(), gameClient.getHabbo(), RoomChatMessageBubbles.ALERT)));
        }
        return true;
    }
    ...
    {
        if(pet.getName().equalsIgnoreCase(this.name))
        {
