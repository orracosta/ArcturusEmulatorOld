...
public class NavigatorPublicFilter extends NavigatorFilter
{
    ...
    public List<SearchResultList> getResult(Habbo habbo)
    {
        ...
        int i = 0;
        resultLists.add(new SearchResultList(i, "official-root", "", SearchAction.NONE, ListMode.THUMBNAILS, DisplayMode.VISIBLE, Emulator.getGameEnvironment().getRoomManager().getPublicRooms(), false, showInvisible));
        i++;
        for (NavigatorPublicCategory category : Emulator.getGameEnvironment().getNavigatorManager().publicCategories.values())
        {
            ...
            {
                resultLists.add(new SearchResultList(i, "", category.name, SearchAction.NONE, category.image, DisplayMode.VISIBLE, category.rooms, true, showInvisible));
                i++;
            }
        }
